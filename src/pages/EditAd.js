...
function EditAd() {
  ...
  // Запрос для получения данных объявления
  ...
  const updateAdMutation = useMutation({
    mutationFn: async (data) => {
      ...
      await updateDoc(doc(db, 'ads', id), {
        ...
        imageUrl,
        location: data.lat && data.lng ? [Number(data.lat), Number(data.lng)] : ad.location,
        updatedAt: new Date(),
      });
    },
    onSuccess: () => {
  ...
  React.useEffect(() => {
    if (ad) {
      reset({
        ...
        lat: ad.location ? ad.location[0] : '',
        lng: ad.location ? ad.location[1] : '',
      });
    }
  }, [ad, reset]);
